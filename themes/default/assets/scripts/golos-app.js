...
var GolosApp = function () {
	...
	var username = null;
    var wif = null;
    var realReviewsArray = new Array();
		      ...
		      submitHandler: function (form) {
		            ...
		            if(checkAuth()){
		                ...
		                steem.broadcast.comment(
		                    ...
		                    '{\"tags\":[\"nsfw\"], \"image\": [\"'+productImage+'\"]}', 
		                function(err, result) {
		                    if(err == null){
		                        jQuery.ajax({
		                            method: "POST",
		                            url: jQuery(form).attr('action'),
		                            data: {'author': username, 'permlink':permlink, 'product':productCode},
		                            ...
		                            steem.api.getContent(username, permlink, function(err, result) {
		                    			if(err == null && result.id){
		                    				...
		                    				realReviewsArray.push({0:result.author, 1:result.permlink});
		                    			}
		                ...
		                $.fancybox.open( jQuery('#loginLink'), {}, jQuery('#loginLink').index( this ) );             
		            }
    ...
    var checkLikeState = function () {
    	if(checkAuth()){
    		Array.prototype.forEach.call(realReviewsArray, function (item) {
                var autor = item[0];
                var permlink = item[1];
                var username = jQuery.cookie('gAuthName');
                steem.api.getActiveVotes(autor, permlink, 
                    function(err, result) {
                        if(err == null){
                            for (i = 0; i < result.length; i++) {
                                if(result[i].voter == username){
                                    jQuery('.' + permlink +' .label').removeClass('label-default').addClass('label-success');
                                    break;
                                }
                            }
                        }
	...
	var createBlockReview = function (result) {
		...
		var html = '<div class="review-item-submitted"><strong>'+result.author+'</strong>'
				 ...
				 +'</ul>'
				 +'<div class="rateit '+result.permlink+'">'
	...
	var createBlockComment = function (comment, type) {
                ...
                var html_body = converter.makeHtml(text);
		
		var li = document.createElement('LI');
		li.className = type;
		var html = '<img class="avatar img-responsive" alt="" src="/themes/default/assets/img/avatar.png"/>'
			...
			+'<span class="body">'+changeYouTubeTag(changeBrTag(html_body))+'</span></div>'
			;
		li.innerHTML = html;
	...
	var dateFormat = function (date) {
        ...
        var changeYouTubeTag = function ( html ) {
            return html.replace( '~~~~~~'    );
        }
        var changeBrTag = function ( html ) {
            return html.replace(/(\r\n\|\r|\n)/gi, "\<br\/\>");
        }
